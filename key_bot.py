...
def server_req(endpoint, method="GET", data=None, retries=3):
    if data is None:
        data = {}
    ...
    for attempt in range(retries):
        try:
            if method == "GET":
                r = requests.get(f"{SERVER_URL}{endpoint}", params=data, timeout=60)
            else:
                r = requests.post(f"{SERVER_URL}{endpoint}", json=data, timeout=60)
            
            if r.status_code == 200 and r.text.strip():
                return r.json()
            else:
                # Server returned empty or non-JSON response (cold start)
                if attempt < retries - 1:
                    print(f"[-] Retry {attempt+1}/{retries} for {endpoint}...")
                    time.sleep(5)
                    continue
                return {"error": "Server returned empty response. Try again."}
        except requests.exceptions.Timeout:
            if attempt < retries - 1:
                print(f"[-] Timeout retry {attempt+1}/{retries} for {endpoint}...")
                time.sleep(5)
                continue
            return {"error": "Server timeout. Render may be waking up. Try again in 30s."}
        except Exception as e:
            if attempt < retries - 1:
                print(f"[-] Error retry {attempt+1}/{retries}: {e}")
                time.sleep(5)
                continue
            return {"error": str(e)}
    
    return {"error": "Server unreachable after retries"}
